...
function debugCenter(context, entity) {
    ...
    var x = entity.x + (isMouse ? entity.scene.camera.x : 0);
    var y = entity.y + (isMouse ? entity.scene.camera.y : 0);
    context.beginPath();
...
var Entity = (function (_super) {
    ...
    function Entity(scene, x, y) {
        ...
        _this_1.on("move:velocity", function () {
            var _a, _b;
            if (!_this_1.isMoving)
                _this_1.isMoving = true;
            _this_1.x += _this_1.vx * _this_1._speed * _this_1.scene.game.secondsPassed;
            _this_1.y +=
                _this_1.vy * (_this_1._gravity || _this_1._speed) * _this_1.scene.game.secondsPassed;
            if ((_b = (_a = _this_1.box) === null || _a === void 0 ? void 0 : _a.parent) === null || _b === void 0 ? void 0 : _b.isActive) {
                if (typeOf(_this_1) === "Circle") {
                    if (_this_1.x < _this_1.radius) {
                        if (_this_1.isMoving)
                            _this_1.isMoving = false;
                        _this_1.vx = Math.abs(_this_1.vx) * restitution;
                        _this_1.x = _this_1.radius;
                    }
                    else if (_this_1.x >
                        _this_1.box.getWidth() - _this_1.radius) {
                        if (_this_1.isMoving)
                            _this_1.isMoving = false;
                        _this_1.vx = -Math.abs(_this_1.vx) * restitution;
                        _this_1.x = _this_1.box.getWidth() - _this_1.radius;
                    }
                    if (_this_1.y < _this_1.radius) {
                        if (_this_1.isMoving)
                            _this_1.isMoving = false;
                        _this_1.vy = Math.abs(_this_1.vy) * restitution;
                        _this_1.y = _this_1.radius;
                    }
                    else if (_this_1.y >
                        _this_1.box.getHeight() - _this_1.radius) {
                        if (_this_1.isMoving)
                            _this_1.isMoving = false;
                        _this_1.vy = -Math.abs(_this_1.vy) * restitution;
                        _this_1.y = _this_1.box.getHeight() - _this_1.radius;
                    }
                }
                else if (typeOf(_this_1) === "Rectangle") {
                    if (_this_1.x < _this_1.width / 2) {
                        if (_this_1.isMoving)
                            _this_1.isMoving = false;
                        if (_this_1.box.rebound)
                            _this_1.vx =
                                Math.abs(_this_1.vx) *
                                    (restitution + (_this_1._bounceWithoutLosingSpeed ? 0.1 : 0));
                        _this_1.x = _this_1.width / 2;
                    }
                    else if (_this_1.x >
                        _this_1.box.getWidth() - _this_1.width / 2) {
                        if (_this_1.isMoving)
                            _this_1.isMoving = false;
                        if (_this_1.box.rebound)
                            _this_1.vx =
                    ...
                    if (_this_1.y < _this_1.height / 2) {
                        ...
                        if (_this_1.box.rebound)
                            _this_1.vy =
                                Math.abs(_this_1.vy) *
                                    (restitution + (_this_1._bounceWithoutLosingSpeed ? 0.1 : 0));
                        _this_1.y = _this_1.height / 2;
                    }
                    else if (_this_1.y >
                        _this_1.box.getHeight() - _this_1.height / 2) {
                        if (_this_1.isMoving)
                            _this_1.isMoving = false;
                        if (_this_1.box.rebound)
                            _this_1.vy =
...
var Game = (function (_super) {
    ...
    function Game(config, w, h) {
        ...
        __spreadArrays(toLoad, ["#all:ended#"]).forEach(function (name, i) {
            if (name === "#all:ended#")
                ...
                config.load[name]
                    ...
                    if (_this.currentScene)
                        _this.currentScene.emit("progress", (i + 1) / toLoad.length);
                })
                    .catch(function (reason) { return _this.globals.emit("e" + Errors.Load, reason); });
        });
        if (!_this.currentScene)
